...
# Create a global ID (starting from 0) for ALL the gridcells

start = process_time()
total_gridcells = total_rows * total_cols
grid_ids = np.linspace(0, total_gridcells, total_gridcells, dtype=int)
...
print("Partiting FSDS/GridID takes  {}".format(end-start))
...
for i in range(number_of_subdomains):
    ...
    os.system('\rm -rf' + subdomain_path+ '/*')
    # Open a new NetCDF file to write the data to. For format, you can choose from
    # 'NETCDF3_CLASSIC', 'NETCDF3_64BIT', 'NETCDF4_CLASSIC', and 'NETCDF4'
